...
export const SettingsPage = () => {
    ...
    try {
      ...
      if (companyDataString) {
        ...
        setCompanyEmail(companyData.props.email || "");
         ...
         toast({ title: "Aviso", description: "Dados da empresa não encontrados localmente.", variant: "default" });
      }
    } catch (error) {
  ...
  const handleDeleteCompany = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "A sua empresa e todos os dados associados foram removidos.",
      });
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      toast({ title: "Utilizador adicionado", description: "Novo utilizador foi criado com sucesso!" });
      setNewUser({ nomeUsuario: "", tipo: "USER", senhaHash: "", empresaId: user.empresaId });
      setIsAddUserDialogOpen(false);
      localStorage.removeItem("companyUsers");
      fetchUsers();
    } catch (error) {
      console.error("Erro ao adicionar utilizador:", error);
      toast({ title: "Erro ao adicionar utilizador", description: "Ocorreu um erro ao criar o utilizador.", variant: "destructive" });
    }
  };
  ...
  const handleDeleteUser = async (id: number) => {
    ...
    try {
      ...
      toast({ title: "Utilizador removido", description: "O utilizador foi removido do sistema." });
      localStorage.removeItem("companyUsers");
      fetchUsers();
    } catch (error) {
      console.error("Erro ao remover utilizador:", error);
      toast({ title: "Erro ao remover utilizador", description: "Ocorreu um erro ao remover o utilizador.", variant: "destructive" });
    }
  };
    ...
    <div className="space-y-6 mt-16 p-4">
      ...
      <Tabs defaultValue="general" className="space-y-6">
        ...
        <TabsContent value="general" className="space-y-6">
          ...
          <Card>
            <CardHeader><CardTitle>Configurações de Feedback</CardTitle></CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label>Envio Automático de Feedback</Label>
                  <p className="text-sm text-muted-foreground">Enviar automaticamente pedidos de feedback após compras</p>
                </div>
          ...
          <div className="flex justify-end">
            <Button onClick={handleSaveSettings}><Save className="w-4 h-4 mr-2" /> Guardar Configurações</Button>
          </div>
            ...
            <CardContent>
              <div className="flex items-center justify-between p-4 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Esta ação é irreversível e removerá todos os dados da sua empresa.</p>
                </div>
                <Dialog>
                  <DialogTrigger asChild><Button variant="destructive"><Trash2 className="w-4 h-4 mr-2" /> Eliminar</Button></DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle className="flex items-center gap-2"><TriangleAlert className="text-destructive" /> Tem a certeza absoluta?</DialogTitle>
                      <DialogDescription className="pt-2">
                        Esta ação não pode ser desfeita. Irá eliminar permanentemente a sua empresa, juntamente com todos os dados associados.
                        <br/><br/>
                        Digite o nome da sua empresa (<b>{companyName}</b>) para confirmar.
                      </DialogDescription>
                    </DialogHeader>
                    <Input
                      onChange={(e) => {
                        ...
                        if (confirmButton) {
                          confirmButton.toggleAttribute('disabled', e.target.value !== companyName);
                        }
                      }}
                    ...
                    <DialogFooter>
                      ...
                      <Button id="confirmDeleteBtn" variant="destructive" onClick={handleDeleteCompany} disabled>
        ...
        <TabsContent value="users" className="space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Faça a gestão de permissões e acesso ao sistema</p>
            </div>
            <Dialog open={isAddUserDialogOpen} onOpenChange={setIsAddUserDialogOpen}>
              <DialogTrigger asChild><Button><Plus className="w-4 h-4 mr-2" /> Novo Utilizador</Button></DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Adicionar Novo Utilizador</DialogTitle></DialogHeader>
                <div className="space-y-4">
                  <div>
                    <Label>Nome de utilizador</Label>
                    <Input value={newUser.nomeUsuario} onChange={e => setNewUser({ ...newUser, nomeUsuario: e.target.value })} placeholder="Nome de utilizador" />
                  </div>
                  <div>
                    <Label>Senha</Label>
                    <Input type="password" value={newUser.senhaHash} onChange={e => setNewUser({ ...newUser, senhaHash: e.target.value })} placeholder="Senha forte" />
                  </div>
                  <div>
                    <Label>Função</Label>
                    <Select value={newUser.tipo} onValueChange={(value: "USER" | "ADMIN") => setNewUser({ ...newUser, tipo: value })}>
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleAddUser} className="flex-1">Adicionar Utilizador</Button>
          ...
          <Card>
            <CardContent className="p-0">
              {users.map((user, index) => (
                <div key={user.id} className={`p-4 flex items-center justify-between hover:bg-muted/50 ${index !== users.length - 1 ? "border-b" : ""}`}>
                  ...
                  <div className="flex gap-2">
                    ...
                    {user.tipo !== "ADMIN" && (
                      <Button variant="outline" size="sm" onClick={() => handleDeleteUser(user.id)}><Trash2 className="w-3 h-3" /></Button>
                    )}
                  </div>
                </div>
              ))}
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">Receber alertas sobre novas avaliações e relatórios</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">Receber notificações em tempo real no navegador</p>
                </div>
